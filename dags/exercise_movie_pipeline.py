...
OMDB_API_KEY = "38023950"  # ⚠️ Reemplaza con tu API key gratuita
...
@dag(
    ...
    schedule="@daily",  # Solo ejecución manual
    doc_md=__doc__,
    default_args={
        "owner": "Universidad",
        "retries": 3,
...
def exercise_movie_pipeline():
    ...
    def fetch_movie_data() -> list[dict]:
        ...
        for movie_title in MOVIES_TO_SEARCH:
            ...
            response = requests.get(OMDB_API_URL, "i=tt3896198&apikey=" + OMDB_API_KEY)
    ...
    def transform_movie_info(raw_movies: list[dict]) -> list[dict]:
        ...
        for movie in raw_movies:
            ...
            clean_movie = {
                ...
                "rating_imdb": float(movie.get("imdbRating", 0.0)),
                "genre": movie.get("Genre", "Desconocido"),
                "plot": movie.get("Plot", "Desconocido"),
            }
    ...
    def display_results(movies: list[dict]) -> None:
        ...
        for movie in movies:
            log.info(movie.get("title") + " " + str(movie.get("rating_imdb", 0)))
